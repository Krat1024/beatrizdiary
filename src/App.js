...
export async function renderApp() {
    ...
    let viewState = 'loading'; // loading, register, login, recovery, main
    ...
    if (!user && viewState === 'loading') {
        viewState = 'register';
    } else if (user && viewState === 'loading') {
        viewState = isSessionActive ? 'main' : 'login';
    }
        ...
        footer.innerHTML = `
            ...
            <div class="social-links">
                ...
                <a href="https://x.com/John1349" target="_blank" title="Twitter">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        <path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.055-4.425 5.055H.316l5.733-6.551L0 .75h5.063l3.495 4.633L12.6.75zm-.86 13.028h1.36L4.323 2.145H2.865l8.875 11.633z" />
                    </svg>
                </a>
            </div>
        `;
    ...
    const updateView = async () => {
        ...
        if (viewState === 'register' || viewState === 'login' || viewState === 'recovery') {
            ...
            if (viewState === 'register') {
                ...
                authContainer.appendChild(renderRecovery({
                    ...
                    onRecovered: () => { viewState = 'login'; updateView(); }
                }));
            } else {
        ...
        // MAIN APP VIEW
        app.innerHTML = `
            <aside id="sidebar" class="glass-panel"></aside>
            <main id="main-content" class="glass-panel"></main>
        `;
        ...
        async function renderDiary() {
            ...
            sidebar.appendChild(renderSidebar({
                ...
                onSelect: (id) => {
                    currentEntryId = id;
                    showSettings = false;
                    mobileMenuOpen = false; // Close on select
                    renderDiary();
                },
                onNew: async () => {
